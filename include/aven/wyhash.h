...
static inline uint64_t aven_wyhash_wyrot_internal(uint64_t x) { return (x>>32)|(x<<32); }
static inline void aven_wyhash_wymum_internal(uint64_t *A, uint64_t *B){
  ...
  *A^=aven_wyhash_wyrot_internal(hl)^hh; *B^=aven_wyhash_wyrot_internal(lh)^ll;
  #else
  *A=aven_wyhash_wyrot_internal(hl)^hh; *B=aven_wyhash_wyrot_internal(lh)^ll;
  #endif
...
static inline uint64_t aven_wyhash_wymix_internal(uint64_t A, uint64_t B){ aven_wyhash_wymum_internal(&A,&B); return A^B; }
...
static inline uint64_t aven_wyhash_wyr8_internal(const uint8_t *p) { uint64_t v; memcpy(&v, p, 8); return v;}
static inline uint64_t aven_wyhash_wyr4_internal(const uint8_t *p) { uint32_t v; memcpy(&v, p, 4); return v;}
#elif defined(__GNUC__) || defined(__INTEL_COMPILER) || defined(__clang__)
static inline uint64_t aven_wyhash_wyr8_internal(const uint8_t *p) { uint64_t v; memcpy(&v, p, 8); return __builtin_bswap64(v);}
static inline uint64_t aven_wyhash_wyr4_internal(const uint8_t *p) { uint32_t v; memcpy(&v, p, 4); return __builtin_bswap32(v);}
#elif defined(_MSC_VER)
static inline uint64_t aven_wyhash_wyr8_internal(const uint8_t *p) { uint64_t v; memcpy(&v, p, 8); return _byteswap_uint64(v);}
static inline uint64_t aven_wyhash_wyr4_internal(const uint8_t *p) { uint32_t v; memcpy(&v, p, 4); return _byteswap_ulong(v);}
#else
...
#endif
static inline uint64_t aven_wyhash_wyr3_internal(const uint8_t *p, size_t k) { return (((uint64_t)p[0])<<16)|(((uint64_t)p[k>>1])<<8)|p[k-1];}
//wyhash main function
static inline uint64_t aven_wyhash(const void *key, size_t len, uint64_t seed, const uint64_t *secret){
  const uint8_t *p=(const uint8_t *)key; seed^=aven_wyhash_wymix_internal(seed^secret[0],secret[1]);	uint64_t	a,	b;
  if(aven_wyhash_likely_internal(len<=16)){
    if(aven_wyhash_likely_internal(len>=4)){ a=(aven_wyhash_wyr4_internal(p)<<32)|aven_wyhash_wyr4_internal(p+((len>>3)<<2)); b=(aven_wyhash_wyr4_internal(p+len-4)<<32)|aven_wyhash_wyr4_internal(p+len-4-((len>>3)<<2)); }
    else if(aven_wyhash_likely_internal(len>0)){ a=aven_wyhash_wyr3_internal(p,len); b=0;}
    else a=b=0;
  }
  else{
    size_t i=len; 
    if(aven_wyhash_unlikely_internal(i>=48)){
      ...
      do{
        seed=aven_wyhash_wymix_internal(aven_wyhash_wyr8_internal(p)^secret[1],aven_wyhash_wyr8_internal(p+8)^seed);
        see1=aven_wyhash_wymix_internal(aven_wyhash_wyr8_internal(p+16)^secret[2],aven_wyhash_wyr8_internal(p+24)^see1);
        see2=aven_wyhash_wymix_internal(aven_wyhash_wyr8_internal(p+32)^secret[3],aven_wyhash_wyr8_internal(p+40)^see2);
        p+=48; i-=48;
      }while(aven_wyhash_likely_internal(i>=48));
      seed^=see1^see2;
    }
    while(aven_wyhash_unlikely_internal(i>16)){  seed=aven_wyhash_wymix_internal(aven_wyhash_wyr8_internal(p)^secret[1],aven_wyhash_wyr8_internal(p+8)^seed);  i-=16; p+=16;  }
    a=aven_wyhash_wyr8_internal(p+i-16);  b=aven_wyhash_wyr8_internal(p+i-8);
  }
  a^=secret[1]; b^=seed;  aven_wyhash_wymum_internal(&a,&b);
  return  aven_wyhash_wymix_internal(a^secret[0]^len,b^secret[1]);
}

//the default secret parameters
static const uint64_t aven_wyhash_wyp_internal[4] = {0x2d358dccaa6c78a5ull, 0x8bb84b93962eacc9ull, 0x4b33a62ed433d4a3ull, 0x4d5a2da51de1aa47ull};

//a useful 64bit-64bit mix function to produce deterministic pseudo random numbers that can pass BigCrush and PractRand
static inline uint64_t aven_wyhash64(uint64_t A, uint64_t B){ A^=0x2d358dccaa6c78a5ull; B^=0x8bb84b93962eacc9ull; aven_wyhash_wymum_internal(&A,&B); return aven_wyhash_wymix_internal(A^0x2d358dccaa6c78a5ull,B^0x8bb84b93962eacc9ull);}
...
static inline uint64_t aven_wyhash_wy2u0k(uint64_t r, uint64_t k){ aven_wyhash_wymum_internal(&r,&k); return k; }
#endif

// modified from https://github.com/going-digital/Prime64 
static	inline	unsigned long long	aven_wyhash_mul_mod(unsigned long long a, unsigned long long b, unsigned long long m) {
...
static inline unsigned long long aven_wyhash_pow_mod(unsigned long long a, unsigned long long b, unsigned long long m) {
...
static inline void aven_wyhash_make_secret(uint64_t seed, uint64_t *secret){
  ...
  for(size_t i=0;i<4;i++){
    ...
    do{
      ok=1; secret[i]=0;
      for(size_t j=0;j<64;j+=8) secret[i]|=((uint64_t)c[aven_wyhahs_rand(&seed)%sizeof(c)])<<j;
