...
static inline uint32_t aven_c_ast_parse_macro_invocation(AvenCAstCtx *ctx) {
    ...
    if (aven_c_ast_match_punctuator(ctx, aven_str("("))) {
        ...
        for (;;) {
            ...
            if (arg == 0) {
                arg = aven_c_ast_parse_type_name(ctx);
            }
            if (arg == 0) {
                break;
            }
...
static inline uint32_t aven_c_ast_parse_string_constant(AvenCAstCtx *ctx) {
    ...
    for (;;) {
        ...
        if (ctx->ppd) {
            ...
            if (part == 0 and count > 0) {
                part = aven_c_ast_parse_punctuator(ctx, aven_str("##"));
                if (part != 0) {
                    last_merge = true;
                }
            } else {
    ...
    // we shouldn't consider a single macro a string literal, it will be parsed
    // as either an identifier, a postfix expression, or a single macro invocation
    if (last_merge or count == 0 or (count == 1 and !paste and literals == 0)) {
        aven_c_ast_restore(ctx, state);
        return 0;
    }
    return aven_c_ast_push(
...
static inline bool aven_c_ast_render_write(AvenCAstRenderCtx* ctx, AvenStr str) {
    ...
    size_t cap = ctx->line.len - (ctx->newline_str.len + offset);
    ...
    if (rem.len < str.len) {
        return aven_str_equals(str, aven_str(" "));
    }
    slice_copy(rem, str);
...
#define aven_c_ast_render_data_try_surround_split(c, t, n, p, b, e, f, s, l) \
    do { \
        if ( \
            !aven_c_ast_render_data_try_surround_internal( \
                c, \
    ...
    if (ctx->io_error == 0) {
        return (AvenCAstRenderResult){
            ...
            .msg = aven_fmt(
                ...
                aven_fmt_uint(ctx->line.len - (ctx->newline_str.len + 2))
            ),
        };
    }
    return (AvenCAstRenderResult){
